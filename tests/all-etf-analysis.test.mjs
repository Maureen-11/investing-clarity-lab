...
test("all 300 ETF histories can execute a factual replay", async () => {
  ...
  for (const entry of manifest.entries) {
    ...
    for (const frequency of ["daily", "monthly", "yearly"]) {
      ...
      assert.ok(Number.isFinite(replay.endValue) && replay.endValue > 0, `${entry.id} ${frequency} ending value is invalid`);
      assert.ok(Number.isFinite(replay.principal) && replay.principal > 0, `${entry.id} ${frequency} principal is invalid`);
      assert.ok(Number.isFinite(replay.productMaxDrawdown), `${entry.id} ${frequency} drawdown is invalid`);
