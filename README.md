...
- 300只主流ETF历史包：精选 A 股150只、美股105只、港股45只，全部按标的加载独立历史；优先使用复权价格，部分A股ETF在价格接口受限时使用更长的累计净值序列，覆盖宽基、红利低波、成长科技、行业、债券、现金、黄金和 REIT。
- 15个市场参照指数：覆盖 A 股7个、美股5个、港股3个主要指数；指数只展示市场统计、年度表现和回撤，不冒充可购买ETF，也不接受定投输入。
- 定投日历：支持每日、每月、每年投入；已公布年份使用交易所休市安排，超出覆盖期明确标为估算。
- 本金统计：计算日历天数、交易日、实际投入次数、分期本金、初始资金和总投入。
- 分层分析：300只ETF按实际可用期限生成定投回放；历史不足用户所选年限时自动使用最长真实窗口，少于1年时只显示累计投入和实际回撤；个股仍进入事实统计模式。
...
- 默认构建使用300只ETF和15个指数的静态日终快照；每周一北京时间10:00自动尝试整包更新。任意一只抓取或校验失败都会保留上一版完整数据。
- 300只ETF均可进行历史回放，但只使用ETF自身实际存在的历史。请求20年而产品只有8年时显示“请求20年 / 实际可用8年”；少于1年不生成年化或多年滚动结论。
- 自动更新不再直推受保护的 `main`，而是创建或更新 `automation/weekly-market-data` Pull Request；通过跨平台测试后仍需人工确认合并。
- ETF历史优先使用东方财富前复权日线，网络限制时回退腾讯证券qfq日线；A股价格接口受限时可采用东方财富产品页累计净值或BaoStock adjustflag=2。每只文件记录实际来源、序列类型和真实可用区间，不会把较短历史补写成完整存续期。累计净值与交易所成交价并非同一概念，页面会明确标出。旧版Yahoo快照只作故障审计。仓库不声称已经获得公开展示或再分发许可；来源说明和风险提示不能替代正式授权。
