...
- **US, mainland China, and Hong Kong security search:** Search stocks and ETFs across the three markets; the market selection follows the chosen security.
...
- **Historical replay:** Generates rolling-start and specified-start results for QQQ, VOO, SPY, VTI, VT, and SCHX when verified adjusted-return history is available.
...
- `npm test` builds the app first and then runs tests for trading days, FX, CPI, historical replay, and page boundaries.
...
This rolling replay contains 11 complete 15-year starting points; the middle-ranked path is `2011-01-03—2026-01-05`. ETF history ends on `2026-07-27`, and macro data ends on `2026-08-18`. The number of samples and results change when the data is updated.

## Data, privacy, and licensing boundaries

- The website uses static data snapshots. It is not a real-time market terminal and does not provide a backend daily-update service.
