...
- **Curated 100-ETF catalogue:** 50 mainland China, 35 US, and 15 Hong Kong ETFs across broad market, dividend/low volatility, growth, sectors, bonds, cash, gold, and REITs. Catalogue verification is shown separately from history availability.
...
- **Layered analysis:** Generates rolling and specified-start replay only when verifiable history is available; individual stocks use a facts-only mode and missing history produces an explicit empty state.
...
| Least favorable historical ending value | ¥659,511 |
| Middle-ranked historical ending value | ¥721,625 |
| Most favorable historical ending value | ¥739,210 |
| Starting purchasing power of the middle-ranked path | ¥561,587 |
| Estimated fee drag on the middle-ranked path | ¥2,266 |
| Maximum drawdown in the VOO sample | -33.99% |

This rolling replay contains 11 complete 15-year starting points; the middle-ranked path is `2011-01-03—2026-01-05`. ETF history still ends on `2026-07-27` (the latest Yahoo request returned 403, so the verified snapshot was preserved); the FX/macro snapshot was refreshed through `2026-08-21`, and the latest published CPI month is `2026-07`. The number of samples and results change when the data is updated.

## Data, privacy, and licensing boundaries

- The default GitHub Pages build uses static snapshots. The optional free-beta CloudBase adapter serves the same snapshots; it is not a real-time market terminal and does not promise daily backend updates.
- All 100 ETF catalogue entries and categories are validated, but only QQQ, VOO, SPY, VTI, VT, and SCHX currently have local history files. Other entries are labelled as catalogue-verified with history pending and never receive fabricated return ranges.
- Adjusted-return ETF history comes from the Yahoo Finance Chart API. This repository **does not claim that it has obtained public-display or redistribution permission**. Listing the source does not itself grant permission; anyone publishing or using the data should assess the applicable terms and obtain any required license.
