...
export default function PlanPage() {
  ...
  const historyLatest = useMemo(() => Object.values(historyLibrary).map((series) => series.lastDate).sort().at(-1), [historyLibrary]);
  const retrievedLatest = useMemo(() => Object.values(historyLibrary).map((series) => series.retrieved).sort().at(-1), [historyLibrary]);
  ...
  }, [channelRates, principal, replay, historySeries, options]);
  ...
  return <main>
    ...
    <section className="channels-section" id="channels"><div className="section-heading"><div><p>FUND COST VERIFIER</p><h2>产品费用和购买渠道，分开算</h2></div><p>ETF年费率已经体现在基金净值里；销售渠道可能影响申购费、佣金、汇兑与点差。支付宝和天天基金没有一个适用于所有产品的统一“默认费率”。</p></div>
      {detail&&replay&&<div className="fee-impact"><article><span>{selected?.symbol} 官方当前年费率</span><strong>{detail.fee}%</strong><p>{detail.issuer}，截至 {detail.feeAsOf}</p></article><article><span>按当前费率估计的直接运营费</span><strong>{compactMoney(replay.median.estimatedDirectFee)}</strong><p>把当前费率应用于整段持仓的近似值，不是账单</p></article><article><span>按当前费率估计的期末总拖累</span><strong>{compactMoney(replay.median.feeDrag)}</strong><p>含失去的复利；不代表历史费率始终不变</p></article></div>}
...
function MetricPanel({ metrics, detail }: { metrics: ProductMetrics; detail?: FundDetail }) {
  const items = [
    { name:"存续期年化总回报", value:`${metrics.cagr.toFixed(1)}%`, help:"把成立至今的复权总回报折成年均速度。它高度依赖起止日期，不是未来预期。" },
...
function ComparisonRows({ a, b, first, second }: { a:string; b:string; first:ProductMetrics; second:ProductMetrics }) {
  const rows = [
    ["存续期年化总回报", `${first.cagr.toFixed(1)}%`, `${second.cagr.toFixed(1)}%`],
